...
```{r data import updated august 2023}

wild <- read_csv(here("data/WI_images3aug2023.csv")) %>% 
  ...
  mutate(day = day(date)) %>% 
  mutate(deployment_id = str_remove(deployment_id, regex(" cam", ignore_case = T))) %>% # remove Cam from site name so it matches other datasets; regex necessary for ignore_case
  ...
  #filter(month != "Dec") %>% not sure why we were removing december but no longer need to
  filter(year <= 2022) # eventually can take this out 

unique(wild$deployment_id)

```
## If we want to take it a step further, we can sort out all of the IDs that didn't get it down to genus or species for accurate species richness analysis

```{r create dataframe of species specific IDs only}

wild_spec <- wild %>% 
  filter(!is.na(species)) %>% 
  filter(species!= "No CV Result")

# make one that allows up to genus

wild_gen <- wild %>% 
  filter(!is.na(genus))%>% 
  filter(genus!= "No CV Result")


# i want the genus specific dataset for later analysis

write_csv(wild_gen, here("data/WI_IDs_togenus.csv"))
