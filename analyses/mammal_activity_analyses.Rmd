...
activity_site_month <- mammals_clean %>% 
  #left_join(mammals_clean, trapnights_month, by = join_by(sitename, month)) %>% # first append monthly trapnights to full dataset ## no longer have to do this since it's in its own col
  group_by(sitename, month, days_active, burst_settings, utm_x, utm_y, dist_jalama) %>% 
