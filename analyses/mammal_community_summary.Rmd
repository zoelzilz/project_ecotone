...
activity_site_month_sp <- left_join(mammals_clean, trapnights_month, by = join_by(sitename, month)) %>% # first append monthly trapnights to full dataset
  ungroup()%>%
  group_by(sitename, month, trapnights, burst_settings, common_name) %>% 
...
spercent_seasonal_activity_plot <- activity_site_month_sp %>% 
  ...
  ggplot(., aes(x = month, y = spp_activity)) +
    geom_bar(aes(fill = common_name), 
             ...
             stat = "identity")+
  scale_fill_manual(values = IDcolors)+
  ylab("Percent of Total Seconds of Animal Activity")+
  xlab("Month (2022") +
  theme_classic()
...
mammalPCA <- rda(count_matrix_expanded) #no env variables in here
mammalPCA
...
#
mammal_NMDS <- metaMDS(count_matrix_expanded)

mammal_NMDS

nmdsplot <- plot(mammal_NMDS) #circles are sites (communities/rows), crosses are species
