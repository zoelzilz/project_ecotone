...
#### add in distance to Jalama (as center of human activity) IN KILOMETERS using package geosphere ####
...
metadata2 <- metadata %>% 
  mutate(dist_jalama = (distVincentyEllipsoid(cbind(utm_y, utm_x), jalama_mtx))/1000) %>% # dist returns shortest distance in meters as default, so outputting km
...
pretty_summary <- kable(camera_activity_summary, #format = "latex"
...
save_kable(pretty_summary, "camera_summary_table.png")
...
seasonal_mean_humans_plot <- deployment_data_humans %>% 
  ...
  xlab("Month") +
  geom_boxplot(aes(color = season, fill = season))+
  theme_classic()+
  scale_y_continuous(limits = c(0, 120))+
  #geom_boxplot(aes(color = season, fill = season), alpha = 0.1, outlier.shape = NA)+ # makes crazy jalama july outlier (600 seconds!) go away haha but also takes away all other dots
  scale_fill_manual(name = "Season", 
                    values = c("wet" = "dodgerblue1", "dry" = "gold")
                      )+
   scale_color_manual(name = "Season", 
                    values = c("wet" = "dodgerblue4", "dry" = "gold3")
                      )

#png(here("figures/humans_season_trunc.png"), width = 10, height = 6, units = "in", pointsize = 12, res = 1000)
...
#########################################################################################
count_matrix_expanded <- activity_site_month_sp %>% # on 9/13/24 we are using the version of this DF loaded from mammal activity analyses because it has the correct # active days, zeros, and relative abundance index (seconds per trapnight) for each month
  ungroup() %>% 
...
count_matrix_byeffort <- activity_site_month_sp %>% 
  ...
  # here's where we should take out "mammal" and "carnivorous mammal"
  filter(common_name != "unidentified mammal") %>% 
  dplyr::select(sitemonth_id, common_name, seconds_by_trapnight) %>%
  
  #group_by(sitename, common_name) %>% # adjusted seconds by site and species
  #summarise(seconds_by_trapnight = sum(seconds_by_trapnight)) %>% # in this version of the df, this column already exists
...
sp.count <- specnumber(count_matrix_byeffort)
sp.freq <- specnumber(count_matrix_byeffort, MARGIN = 2) # relative frequency of each sp. compared to others
...
species_plot <- ggplot(sp_df, aes(x= sitename, y = value)) +
  ...
  geom_boxplot(aes(fill = dist_jalama))+
  scale_fill_viridis_b()+
  theme_classic()+  
  ylab("Mean Species Richness")+
  xlab("Camera Stations, North to South")+
  theme(
...
#png(here("figures/richness_site.png"), width = 10, height = 8, units = "in", pointsize = 12, res = 1000)
#species_plot
#dev.off()
...
shannondiv <- diversity(count_matrix_byeffort) 
...
mammal_perm
#mammal_pairwise_site <- pairwise.adonis(count_matrix_expanded, vegan_metadata$sitename)


mammal_perm_season <- adonis2(count_matrix_expanded ~ month, data = vegan_metadata)
mammal_perm_season 

# theres also a pairwise comparison tool
library(devtools)
install_github("pmartinezarbizu/pairwiseAdonis/pairwiseAdonis")
library(pairwiseAdonis)

mammal_pairwise_season <- pairwise.adonis(count_matrix_expanded, vegan_metadata$season)
#mammal_pairwise_month <- pairwise.adonis(count_matrix_expanded, vegan_metadata$month)
...
bray_curtis_diss <- as.matrix(vegdist(count_matrix))
#
mammal_NMDS <- metaMDS(count_matrix, trace = FALSE, trymax = 200)
                       #, k = 2, # k is the number of dimensions
...
plot(mammal_NMDS, type = "n") %>%  #circles are sites (communities/rows), crosses are species
  points("sites", cex = 0.8) %>% 
  text("species")
...
mammals_vars_fit <- envfit(mammal_NMDS, metadata2, permutations = 999)
mammals_sp_fit <- envfit(mammal_NMDS, count_matrix, permutations = 999)

# exracting outputs

# site vars

site_scores <- as.data.frame(scores(mammal_NMDS, display = "sites"))
site_scores <- cbind(site_scores, property = metadata2$property)
site_scores <- cbind(site_scores, iz_type = metadata2$iz_type)

# sp vars
sp_scores <- as.data.frame(scores(mammals_sp_fit, display = "vectors")) # spp intrinsic values saved to df
sp_scores <- cbind(sp_scores, species = rownames(sp_scores)) # make species names rownames? syntax is ew
sp_scores <- cbind(sp_scores, pval = mammals_sp_fit$vectors$pvals) # pvals to select significant?? spp

# environmental variables
env_scores <- as.data.frame(scores(mammals_vars_fit, display = "vectors"))
env_scores <- cbind(env_scores, env_vars = rownames(env_scores))
env_scores <- cbind(env_scores, pval = mammals_vars_fit$vectors$pvals) 

######## plotting! ##########

mammalsNMDSplot <- ggplot(site_scores, aes(NMDS1, NMDS2)) +
  geom_point(aes(NMDS1, NMDS2, colour = factor(property), shape = factor(iz_type)))+
  coord_fixed()+
  theme_classic() +
  
  # species vectors (usually you only add "signficant" ones but i put them all on)
  geom_segment(sp_scores, mapping = aes( x = 0, xend = NMDS1, y = 0, yend = NMDS2), 
               arrow = arrow(length = unit(0.25, "cm")),
               colour = "grey10",
               lwd = 0.3) +
  geom_text_repel(sp_scores, mapping = aes(x=NMDS1, y=NMDS2, label = species))

######## graveyard ##########
#metadata_exp <- activity_site_month_sp %>% 
#  filter(common_name != "unidentified mammal")

#nmds_df <- as_tibble(mammal_NMDS$points) %>% 
  # bind with metadata to plot
#  bind_cols(metadata2, .) # metadata2 if not using expanded matrix

#nmds_plot <- ggplot(nmds_df, aes(x = MDS1, y = MDS2, color = property, shape = property)) +
...
mammalCCA <- cca(count_matrix ~ dist_jalama + iz_type + property, data = metadata2)
mammalCCA
...
# we can run a CCAanova to see which constraints are significant
anova(mammalCCA)
