...
cams <- read_csv(here("data/site_metadata.csv")) %>% 
  clean_names() %>% 
  select(property, utm_y, utm_x, habitat_adjacent, habitat_secondary, iz_type) %>% 
  filter(!property == "HROA")
...
basemap <- get_map(location = c(-120.51, 34.49), zoom = 12, maptype = "satellite", size = c(1000, 1000))

sitemap <- ggmap(basemap) +
  geom_point(aes(x = utm_y, y = utm_x, fill = property), size = 2, colour = "white", shape = 21, data = cams)
  scale_colour

justsites <- ggplot() +
  geom_point(aes(x = utm_x, y = utm_y, size = 10, colour = location), data = cams) +
  theme_void()

ggsave('sitemap_sat.png', sitemap)
...
humans_mammals <- humans_by_site %>% 
  ...
  group_by(deployment_id) %>% 
  mutate(human_rank = human_count[which.max(human_count)]) %>% # this works for some reason
  ungroup() %>% 
  mutate(human_rank = dense_rank(human_rank)) 
  #mutate(human_rank = dense_rank(human_count, ties.method = "first"))
...
humanmammals_bar <- ggplot(humans_mammals, aes(human_rank, total_mammals)) +
  geom_bar(stat= "identity")
...
humanmammals_stacked <- ggplot(humans_mammals, aes(human_rank, n)) +
  geom_bar(stat = "sum", aes(fill = ID)) +
  scale_fill_manual(values = IDcolors) +
  theme_classic()

## work in progress... may not include this guy or just run some anovas idk

ggsave("stacked_by_people.png", humanmammals_stacked)
...
##### % of behaviors that is food related #####

all_behavior <- mammal_behavior %>% 
  group_by(behavior1) %>% 
  summarise(sum(n)) %>% 
  rename(totals = 'sum(n)')

# i cannot for the life of me figure out how to hardcode a conditional sum

#eating <- if(all_behavior$behavior1 == c("eating", "foraging")){
#             sum(all_behavior$totals)}

# the clunkiest way that I hate:
ugh <- sum(all_behavior[which(all_behavior[,1] == "carrying food" | all_behavior[,1] == "eating" | all_behavior[,1] == "foraging" | all_behavior[,1] == "hunting"  ),2])
# syntax : apply sum to condition - (dataframe[which(dataframe[,col#] *condition or series of conditions*), ,col# to sum])

total <- sum(all_behavior$totals)

percent_food_behaviors <- ugh/total
...
ggsave('behavior_stacked.png', behavior_stacked, width = 12, height = 7, bg='transparent')
